/*********************************************************************
** Program Name: Euler11 - Largest product in a grid
...
int main(){
  ...
  // load grid
  stringstream gridStr;
  gridStr << "08 02 22 97 38 15 00 40 00 75 " <<
  ...
  // variables for finding largest product
  int largeProduct = 0;
  int temp = 0;

  // load the 2D array grid from the grid string
  for(int i = 0; i < 20; i++){
    for(int j = 0; j < 20; j++){
      gridStr >> grid[i][j];
    }
  ...
  // test grid horizontally
  for(int i = 0; i < 16; i++){
    for(int j = 0; j < 20; j++){
      temp = grid[i][j] * grid[i+1][j] * grid[i+2][j] * grid[i+3][j];

      if(temp > largeProduct){
        largeProduct = temp;
      }
    }
  }

  // test grid vertically
  for(int i = 0; i < 20; i++){
    for(int j = 0; j < 16; j++){
  ...
  // test grid diagonally first high
  for(int i = 0; i < 16; i++){
    for(int j = 0; j < 16; j++){
  ...
  // test grid diagonally last high
  for(int i = 0; i < 16; i++){
    for(int j = 0; j < 16; j++){
